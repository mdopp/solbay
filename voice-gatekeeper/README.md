...
Solilos-published Python image that bridges Wyoming-protocol satellites (HA Voice PE, `wyoming-satellite` CLI) to Hermes. Runs as a container inside Solilos's `solbay` pod; reaches ServiceBay's unchanged `voice` template (Whisper + Piper + openWakeWord) via host loopback. Both pods are `hostNetwork: true`, sharing the host netns. The `GATEKEEPER_IMAGE` variable on `solbay` picks which image tag to run.
...
Gatekeeper
  ...
  → Hermes (HTTP, solbay neighbour pod): converse(text, uid, endpoint, location, trace_id)
...
| **4** | Multi-room routing (response goes to the satellite the user is closest to), voice-tone sensor parallel to STT, custom "Sol" wakeword. |
...
| `SOLILOS_DB_PATH` | `/var/lib/solilos/solilos.db` | SQLite file (Phase 2: `voice_embeddings` read/write) |
| `SOLILOS_SPEAKER_ID_ENABLED` | empty | Set to `1`/`true` to turn on Phase-2 speaker resolution. Off by default — the stock image has no ECAPA model anyway. |
| `SOLILOS_SPEAKER_ID_THRESHOLD` | `0.55` | Cosine similarity threshold above which a k-NN match is accepted. Below it, the resolver falls back to `DEFAULT_UID` (and logs the score). |
| `SOLILOS_SPEAKER_MODEL_CACHE` | `/var/lib/solilos/models/spkrec-ecapa` | Where SpeechBrain caches the pretrained ECAPA weights. ~80 MB on first start. |
| `SOLILOS_DEBUG_MODE` | `false` | Initial verbose-mode default (runtime override comes from `system_settings.debug_mode` in `solilos.db`) |
...
1. Build a custom gatekeeper image that installs the extras:
   ...
   ```dockerfile
   FROM ghcr.io/mdopp/solilos-gatekeeper:latest
   RUN pip install --no-cache-dir 'solilos-gatekeeper[speaker-id]'
   ```

2. Point `GATEKEEPER_IMAGE` (the `solbay` template variable)
   at your image tag.
3. Set `SOLILOS_SPEAKER_ID_ENABLED=1` in the wizard.
...
`voice_pe_rooms` table of `solilos.db`. When unknown, `location` is `null`
...
**Interim store.** `solilos.db` holds the mapping for now. The longer-term
...
HERMES_URL=http://localhost:8642 SOLILOS_DEBUG_MODE=true gatekeeper
```
...
Built from this directory by [`.github/workflows/build-images.yml`](../.github/workflows/build-images.yml) on every push to `main` and on tags. Published as `ghcr.io/mdopp/solilos-gatekeeper:latest`. To rebuild locally: `podman build -t ghcr.io/mdopp/solilos-gatekeeper:latest voice-gatekeeper/` (from the repo root).
...
Architecture: [`../../../../solilos-architecture.md`](../../../../solilos-architecture.md) → "gatekeeper (Solilos-published image)".
